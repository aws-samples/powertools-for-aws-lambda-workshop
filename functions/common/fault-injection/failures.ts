...
const latencyFailure = async (min: number, max: number): Promise<number> => {
  let latencyRange = max - min;
  let setLatency = Math.floor(min + Math.random() * latencyRange);
...
const denyListFailure = (denylist: string[], mitmHandler: any) => {
  ...
  mitmHandler.on("connect", function (socket: any, opts: any) {
    let block = false;
    blockRegexes.forEach(function (blRegex) {
