...
def get_original_object(s3_client, object_key: str, bucket_name: str):
    ...
    try:
        ...
        raise Exception(f"Error getting file from S3 -> {str(e)}")  # Handle or log the exception as needed
...
def write_thumbnail_to_s3(s3_client, object_key: str, bucket_name: str, body: bytes=None):
    try:
        file_body = body
