...
from constants import TRANSFORMED_IMAGE_PREFIX, TRANSFORMED_IMAGE_EXTENSION, TRANSFORM_SIZE
...
@idempotent_function(persistence_store=persistence_layer, config=idempotency_config, data_keyword_argument="event")
def process_thumbnail(event: dict, object_key: str, object_etag:str):
    new_thumbnail_key: str = f"{TRANSFORMED_IMAGE_PREFIX}/{uuid.uuid4()}{TRANSFORMED_IMAGE_EXTENSION}"

    logger.info(f"Generate Thumbnail for Object Key: {object_key} and Etag: {object_etag}");

    original_image: bytes = get_original_object(s3_client=s3_client,object_key=object_key,bucket_name=s3_bucket_files)

    thumbnail_size = TRANSFORM_SIZE.get("SMALL")

    thumbnail_image = create_thumbnail(original_image=original_image, width=thumbnail_size.get("width"), height=thumbnail_size.get("height"))

    write_thumbnail_to_s3(s3_client=s3_client, object_key=new_thumbnail_key, bucket_name=s3_bucket_files,body=thumbnail_image)
...
def lambda_handler(event, context: LambdaContext):
    ...
    try:
        thumbnail_object_key = process_thumbnail(event=event, object_key=object_key, object_etag=object_etag)
